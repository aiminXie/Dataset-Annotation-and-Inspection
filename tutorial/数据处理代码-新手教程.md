...
> 一般细胞数在4000以上就需要存成稀疏矩阵，这样方便后续的操作，因为后面绝大多数的自动函数都需要读取TPM矩阵，保存常规矩阵会运行得特别慢。
...
>特别**不赞成**将coo_matrix转化为dataframe，因为一般作者提供了coo_matrix则说明该数据特别大，处理成dataframe的内存压力过大。可以使用上面情况2.1的例子处理coo_matrix，除非你需要对矩阵做特别复杂的操作（一般不常见，如果遇到也请尽量对coo_matrix进行操作）。此方式只是下下策。
>
```python
from scipy import sparse
from scipy.sparse import coo_matrix
